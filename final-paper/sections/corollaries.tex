...
For $k\ge2$, the fixed-degree identity \eqref{eq:g-F} gives
...
\section{Abelian factorization of the period-4 delta factors}
...
\]
Restricting the action to $\Gal(\overline\Q/B)$ preserves equivariance,
so $\beta$ identifies Galois orbits over $B$, including their
cardinalities.  Since the irreducible factors of a separable polynomial
correspond exactly to these orbits, Theorem
\ref{thm:abelian-factorization} transfers without requiring an explicit
...
Let $k\ge2$ and $\ell\ge1$ be integers, and let $\zeta_\ell$ be a
primitive $\ell$-th root of unity.  If $\gcd(k,\ell)=1$, then both $\Gamma_k$ and
$\widetilde\Delta_{4k,4}$ remain irreducible over
$\Q(\zeta_\ell)$.
\end{corollary}

\begin{proof}
For coprime $k$ and $\ell$, one has
$K_k\cap\Q(\zeta_\ell)=\Q$.  Apply Theorems
\ref{thm:abelian-factorization} and
\ref{thm:delta-abelian-factorization} with
$B=\Q(\zeta_\ell)$.
\end{proof}

\begin{remark}
Corollary~\ref{cor:coprime-cyclotomic-base} is the period-$4$ analogue
of the coprime cyclotomic-base result for period $3$ in
\cite[Theorem 1.2]{KoizumiMurakamiSanoTakehira2025}.  The full
factorization theorems above go further in the period-$4$ setting by
allowing every finite abelian base field and describing all resulting
factors.
\end{remark}
...
Let $C=4c$ be an exact-period-$4$ parabolic parameter in the scaled
coordinate, with primitive $k$-th root multiplier $\zeta$, where
$k\ge2$.  Then
\[
...
$S_3$, and the cubic field $\mathbf Q(C)$ is not Galois over
$\mathbf Q$.  Every finite subfield of $\mathbf Q^{\mathrm{ab}}$ is Galois over
...
\cite[Chapter VI, \S9, Theorem 9.1]{Lang2002} in this setting states that
