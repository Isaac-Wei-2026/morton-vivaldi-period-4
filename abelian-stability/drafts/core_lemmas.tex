...
The proof in the case $m>1$ uses the following two local statements,
which were established in the companion paper \cite{WeiPeriod4}.  Choose
an odd-order root of unity $\theta\in K$ such that $\theta^4=\eta$, and
put $t_0=\theta-1$.  Then $t_0$ is a $\mathfrak p$-adic unit and
\begin{equation}
 \overline{F_\zeta(T)}=T^2(T-\overline{t_0})^4,
 \qquad
 v\bigl(F_\zeta(t_0)\bigr)=1.
 \label{eq:quartic-established-odd-data}
\end{equation}
These are the only previously established local fiber statements used
in the odd-part argument below.
...
\]
The second assertion in \eqref{eq:quartic-established-odd-data} gives
$v(b_0)=1$.  Reducing the shifted polynomial and using the first
assertion in \eqref{eq:quartic-established-odd-data}, we obtain, in
...
\[
 D=E\cap k^{\mathrm{ab}}.
\]
Because $D\subseteq E$, the extension $D/k$ is finite.  Moreover,
$k^{\mathrm{ab}}/k$ is Galois with abelian Galois group.  Every finite
intermediate extension is therefore Galois and abelian over $k$; in
particular, so is $D/k$.  Since $f$ is irreducible of degree six,
...
the equality of degrees give $D=E$.  In particular, $E/k$ is abelian
Galois.  With $q$, $\beta$, and $\sigma$ as in the preceding paragraph,
we have
\[
...
We have proved $D=k$.  It remains to justify the assertion over the
infinite extension $k^{\mathrm{ab}}$.  If $f$ were reducible over
